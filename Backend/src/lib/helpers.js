...
helpers.resizeImage =  (files) => {
  ...
  helpers.insertarAdviserSiNoExiste= async (id, nombre, email)=> {
   try {
    const query = `
      INSERT INTO advisers (id_adviser, adviser_name, adviser_email, adviser_username, adviser_password, adviser_role, adviser_lastname, adviser_rut, adviser_phone, adviser_birthday)
      SELECT ?, ?, ?, '', '', '', '', '', '', CURDATE()
      FROM DUAL
      WHERE NOT EXISTS (
    ...
    const [result] = await pool.query(query, [id, nombre, email, id]);
console.log(query)
    if (result.affectedRows > 0) {
      console.log('Adviser insertado exitosamente.');
    } else {
      console.log('El adviser ya existe. No se insertó.');
    }
  } catch (err) {
    console.error('Error al insertar adviser:', err);
  } finally {
    pool.release();
  }
}
