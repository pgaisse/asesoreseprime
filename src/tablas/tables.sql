...
CREATE TABLE IF NOT EXISTS `cub`.`casos` (
  ...
  INDEX `fk_casos_clients1_idx` (`id_client` ASC),
...
CREATE TABLE IF NOT EXISTS `cub`.`damages` (
  ...
  `createdAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `casos_id_caso` INT(11) NOT NULL,
  PRIMARY KEY (`id_damage`),
  INDEX `fk_damages_casos1_idx` (`casos_id_caso` ASC),
  CONSTRAINT `fk_damages_casos1`
    FOREIGN KEY (`casos_id_caso`)
    REFERENCES `cub`.`casos` (`id_caso`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
CREATE TABLE IF NOT EXISTS `cub`.`repairs` (
  ...
  `createdAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `damages_id_damage` INT(11) NOT NULL,
  PRIMARY KEY (`id_repair`),
  INDEX `fk_repairs_damages1_idx` (`damages_id_damage` ASC),
  CONSTRAINT `fk_repairs_damages1`
    FOREIGN KEY (`damages_id_damage`)
